...
// CLARITY Universal App — Interview module I1 v1.0.0
// Chat Q10 is released. Audio, video and mix files are registered as the
// controlled foundations for I2–I4 but remain unavailable through Workspace.

const FRAME_BY_MODE = Object.freeze({
  chat: './modules/interview-chat.html?v=1.0.0',
...
function errorShape(error){return{code:String(error?.code||'INTERVIEW_MODULE_ERROR'),message:String(error?.message||error||'Interview module request failed.'),details:error?.details||null}}
...
export function createInterviewModule(ctx){
  ...
  function initFrame(){if(!ready){pendingInit=true;return}pendingInit=false;send('INIT',{uid:state.uid,token:state.token,locale:getLocale(),runtime:runtime(),branding:state.payload?.branding||{},release:{chat:true,audio:false,video:false,mix:false}})}
  async function action(name,payload={}){const endpoint={status:'v2InterviewStatus',start:'v2InterviewStart',message:'v2InterviewMessage',finish:'v2InterviewFinish',retry:'v2InterviewRetry'}[name];if(!endpoint)throw Object.assign(new Error(`Unknown Interview action '${name}'.`),{code:'INTERVIEW_ACTION_UNKNOWN'});return api(endpoint,{body:{token:state.token,uid:state.uid,...payload}})}
