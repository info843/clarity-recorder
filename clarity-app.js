...
const TEXT = {
  de: {
    ...
    productModule:'Produktmodul', moduleText:'Der Universal-App-Router hat die korrekte Produktkonfiguration geladen.', moduleNotice:'Dieses V2-10A-Paket prüft Standalone-Link, gemeinsames Profil, Einwilligung, Branding und den responsiven Router. Das ausführbare Produktmodul wird in der nächsten Vertical-Phase aktiviert.', refresh:'Status aktualisieren', start:'Workflow starten',
    ...
    noInfo:'Keine Angabe', questions:(n)=>n?`Q${n}`:'Standard', responsive:'Responsive Shell', routerReady:'Router bereit', modulePending:'Produktmodul folgt', profileError:'Bitte füllen Sie Vorname, Nachname und eine gültige E-Mail-Adresse aus.', consentError:'Bitte bestätigen Sie alle erforderlichen Erklärungen.', genericError:'Der Vorgang konnte nicht abgeschlossen werden.'
  },
  en: {
    ...
    productModule:'Product module', moduleText:'The Universal App router has loaded the correct product configuration.', moduleNotice:'This V2-10A package validates the standalone link, common profile, consent, branding and responsive router. The executable product module is activated in the next vertical phase.', refresh:'Refresh status', start:'Start workflow',
...
function applyLocale(){
  ...
  Object.entries(map).forEach(([id,k])=>{if($(id))$(id).textContent=t(k)});
  renderPayload();
}
function applyBranding(branding={}){
  const colors=branding.colors||{};const root=document.documentElement;
  if(colors.primary)root.style.setProperty('--navy',colors.primary);if(colors.accent)root.style.setProperty('--cyan',colors.accent);if(colors.secondary)root.style.setProperty('--violet',colors.secondary);if(colors.highlight)root.style.setProperty('--magenta',colors.highlight);
  $('brandName').textContent=branding.brandName||'CLARITY';$('productLine').textContent=branding.legalProductName||'CLARITY Decision Intelligence Platform';
  if(branding.logo){$('brandLogo').src=branding.logo;$('brandLogo').onerror=()=>{$('brandLogo').src='./assets/clarity-logo.png'}}
}
function renderPayload(){
  if(!state.payload)return;const r=state.payload.runtime||{},b=state.payload.branding||{},m=state.module||resolveModuleView(state.payload,state.locale);
  state.module=m;
  $('heroProduct').textContent=m.title||'CLARITY';$('heroDescription').textContent=m.description||'';$('moduleIcon').textContent=m.icon||'CL';$('finalModuleIcon').textContent=m.icon||'CL';
  $('moduleTitle').textContent=m.title||'CLARITY';$('moduleDescription').textContent=m.description||'';$('finalModuleTitle').textContent=m.title||'CLARITY';$('finalModuleDescription').textContent=m.description||'';
  ...
  $('moduleReadiness').textContent=m.startEnabled?t('routerReady'):t('modulePending');$('moduleDevice').textContent=t('responsive');$('finalWorkflow').textContent=r.workflowKey||'—';$('finalStatus').textContent=m.readiness||'—';$('startModuleBtn').disabled=!m.startEnabled;
  $('mediaConsentRow').classList.toggle('hidden',!m.media);applyBranding(b);
}
async function api(path,{method='POST',body=null,query={}}={}){
  const url=new URL(`${API_BASE}/${path}`);Object.entries(query).forEach(([k,v])=>{if(v!==undefined&&v!==null&&v!=='')url.searchParams.set(k,v)});
  const response=await fetch(url,{method,headers:{'Content-Type':'application/json','Accept':'application/json'},body:body?JSON.stringify(body):undefined,credentials:'omit',cache:'no-store'});
  const data=await response.json().catch(()=>({}));if(!response.ok||data?.ok===false){const e=new Error(data?.error?.message||data?.message||`HTTP ${response.status}`);e.code=data?.error?.code||data?.code||`HTTP_${response.status}`;throw e}return data;
}
async function bootstrap(){
  if(!state.uid)return fail({code:'UNIVERSAL_UID_REQUIRED',message:t('unavailableText')});
  try{
    const data=await api('v2AppBootstrap',{method:'GET',query:{uid:state.uid,userAgent:navigator.userAgent,deviceClass:deviceClass()}});state.payload=data;state.token=data.token||'';state.locale=(data.runtime?.participantLang==='de'?'de':'en');state.module=resolveModuleView(data,state.locale);prefill();applyLocale();setStep('welcome');show('welcomeView');
  }catch(e){fail(e)}
}
...
async function saveProfile(){
  const profile={firstName:$('firstName').value.trim(),lastName:$('lastName').value.trim(),email:$('email').value.trim(),externalReference:$('externalReference').value.trim()};
  if(!profile.firstName||!profile.lastName||!/^\S+@\S+\.\S+$/.test(profile.email))return alert(t('profileError'));
  $('saveProfileBtn').disabled=true;try{const data=await api('v2AppProfile',{body:{token:state.token,uid:state.uid,profile}});state.payload.runtime={...state.payload.runtime,...data.runtime};setStep('consent');show('consentView')}catch(e){alert(e.message||t('genericError'))}finally{$('saveProfileBtn').disabled=false}
}
async function saveConsent(){
  const mediaOk=!state.module?.media||$('mediaConsent').checked;if(!$('privacyConsent').checked||!$('aiConsent').checked||!mediaOk)return alert(t('consentError'));
  $('saveConsentBtn').disabled=true;try{const data=await api('v2AppConsent',{body:{token:state.token,uid:state.uid,accepted:true,privacyAccepted:true,aiInteractionAccepted:true,mediaAccepted:state.module?.media?true:false,version:'universal-consent-v1'}});state.payload.runtime={...state.payload.runtime,...data.runtime};renderPayload();setStep('module');show('moduleView')}catch(e){alert(e.message||t('genericError'))}finally{$('saveConsentBtn').disabled=false}
}
...
function wire(){
  $('langBtn').addEventListener('click',()=>{state.locale=state.locale==='de'?'en':'de';applyLocale()});$('supportBtn').addEventListener('click',()=>{location.href='mailto:info@clarity-nvl.com'});
  $('continueProfileBtn').addEventListener('click',()=>{setStep('profile');show('profileView')});$('backWelcomeBtn').addEventListener('click',()=>{setStep('welcome');show('welcomeView')});$('saveProfileBtn').addEventListener('click',saveProfile);$('backProfileBtn').addEventListener('click',()=>{setStep('profile');show('profileView')});$('saveConsentBtn').addEventListener('click',saveConsent);$('statusBtn').addEventListener('click',refreshStatus);
  $('startModuleBtn').addEventListener('click',()=>{alert(state.locale==='de'?'Das Produktmodul wird in der nächsten Vertical-Phase aktiviert.':'The product module is activated in the next vertical phase.')});
}
wire();applyLocale();bootstrap();
