...
import { createInterviewModule } from './modules/interview-module.js?v=1.3.0-i2-standard-controlled-supplement-flow';
...
const TEXT = {
  de: {
    ...
    productModule:'Produktmodul', moduleText:'Der Universal-App-Router hat die korrekte Produktkonfiguration geladen.', moduleNotice:'CV Analysis, Video Presentation, AI Literacy, Assessment sowie Interview Chat Q10 und Audio Q10 sind als Standalone-Module aktiviert. Video und Mix bleiben bis zu ihrer E2E-Freigabe gesperrt.', refresh:'Status aktualisieren', start:'Workflow starten',
  ...
  en: {
    ...
    productModule:'Product module', moduleText:'The Universal App router has loaded the correct product configuration.', moduleNotice:'CV Analysis, Video Presentation, AI Literacy, Assessment, Interview Chat Q10 and Interview Audio Q10 are enabled as standalone modules. Video and Mix remain locked until their E2E release.', refresh:'Refresh status', start:'Start workflow',
...
function resolveBrandLogoSource(branding={}){const values=[branding.logoUrl,branding.logoPublicUrl,branding.publicLogoUrl,branding.customerLogo,branding.logo,branding.logoAsset,branding.media,branding.metadata?.logoPublicUrl,branding.metadata?.logoFileUrl];for(const value of values){for(const candidate of assetCandidates(value)){const url=normalizeWixImageUrl(candidate);if(url)return url}}return''}
...
async function applyBranding(branding={}){const colors=branding.colors||{},root=document.documentElement,mode=String(branding.brandingMode||'clarity').toLowerCase().replace(/[\s-]+/g,'_'),white=mode==='white_label';if(colors.primary)root.style.setProperty('--navy',colors.primary);if(colors.accent)root.style.setProperty('--cyan',colors.accent);if(colors.secondary)root.style.setProperty('--violet',colors.secondary);if(colors.highlight||colors.magenta)root.style.setProperty('--magenta',colors.highlight||colors.magenta);$('brandName').textContent=branding.brandName||(white?'':'CLARITY');$('productLine').textContent=white?(branding.candidatePageSettings?.productLine||'Decision Intelligence Platform'):(branding.legalProductName||'CLARITY Decision Intelligence Platform');const logo=resolveBrandLogoSource(branding);const logoEl=$('brandLogo');logoEl.hidden=true;logoEl.removeAttribute('src');logoEl.style.visibility='visible';let appliedLogo='';if(logo){try{await preloadImage(logo);appliedLogo=logo}catch(error){console.warn('[CLARITY APP] Branding asset fallback',error)}}if(!appliedLogo&&!white){try{await preloadImage('./assets/clarity-logo.png');appliedLogo='./assets/clarity-logo.png'}catch(_){}}if(appliedLogo){logoEl.src=appliedLogo;logoEl.alt=`${branding.brandName||'CLARITY'} logo`;logoEl.hidden=false;const favicon=$('appFavicon');if(favicon)favicon.href=appliedLogo}else{logoEl.hidden=true}return{mode,whiteLabel:white,logoUrl:appliedLogo,assetFallback:Boolean(logo&&!appliedLogo)}}
