...
import { createAssessmentModule } from './modules/assessment-module.js?v=2.27.0-e2-5-assessment-start-language-freeze';
import { createInterviewModule } from './modules/interview-module.js?v=1.9.0-i4-4-1-closeout-loop-recovery';
...
function resolveBrandLogoSource(branding={}){const values=[branding.logoUrl,branding.logoPublicUrl,branding.publicLogoUrl,branding.customerLogoUrl,branding.companyLogoUrl,branding.customerLogo,branding.companyLogo,branding.primaryLogo,branding.logo,branding.logoAsset,branding.media,branding.metadata?.logoPublicUrl,branding.metadata?.logoFileUrl];for(const value of values){for(const candidate of assetCandidates(value)){const url=normalizeWixImageUrl(candidate);if(url)return url}}return''}
...
function renderPayload(){
  ...
  if(isInterview){$('moduleViewText').textContent=state.locale==='de'?'Das Interview ist bereit.':'The Interview is ready.';$('moduleNotice').textContent=state.locale==='de'?'Öffnen Sie das Interview, prüfen Sie Kamera beziehungsweise Mikrofon und starten Sie anschließend.':'Open the Interview, check your camera or microphone once, and then start.';$('startModuleBtn').textContent=state.locale==='de'?'Interview öffnen':'Open Interview'}
  applyBranding(b);
}
